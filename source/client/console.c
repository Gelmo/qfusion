...
void Con_DrawConsole( void ) {
	...
	scaled = 2 * pixelRatio;
	SCR_DrawFillRect( 0, lines - scaled, viddef.width, scaled, colorBlue );
	...
	SCR_DrawString( viddef.width - SCR_strWidth( version, cls.consoleFont, 0, 0 ) - scaled,
					...
					ALIGN_LEFT_TOP, version, cls.consoleFont, colorBlue, 0 );
	...
	if( con.display ) {
		...
		for( x = 0; x < con.linewidth; x += 4 )
			SCR_DrawRawChar( ( x + 1 ) * width, y, '^', cls.consoleFont, colorBlue );
